...
TOXIC_REPLIES = [
    ...
    "Sun le randi ke bacche, tu itna bekaar hai ki teri maa bhi regret karti hogi tune paida hone ka. Tera baap bhi nahi pata hoga tujhe kisne banaya. Ab yahan aa ke bakchodi karta hai? Teri gaand mein itna bada danda ghusa dunga ki 7 din tak toilet nahi kar payega. Samjha kya, gandu?",
    
    "Haan bol na madarchod, kya scene hai? Teri zindagi itni fuckup hai ki main bhi dekh ke has raha hoon. Teri gf roz kisi aur ke saath leti hogi. Chutiya sala.",
    
    "Bina soche jawab: Teri maa ki chut, teri behen ki chut, teri puri family ki gaand. Sab mar jaayenge agar main serious ho gaya.",
]

def get_toxic_reply(message_text: str = ""):
    text = (message_text or "").lower()
    if any(word in text for word in ["hi", "hello", "kaise", "kese", "kya", "bol"]):
        return random.choice(TOXIC_REPLIES) + "\n\nAur bol na chutiye, kya scene hai aaj?"
    
    if random.random() < 0.3:
        return random.choice([r for r in TOXIC_REPLIES if len(r) > 100])
    return random.choice(TOXIC_REPLIES)
...
async def toxic_private(client, message: Message):
    try:
        ...
        print(f"[TOXIC] Reply to {message.from_user.first_name if message.from_user else 'Unknown'}")
...
if __name__ == "__main__":
    print("🚀 Toxic Savage Userbot Started (No Limits, Bina Sochke Mode)")
    print("Warning: Bahut dangerous hai, account ban ho sakta hai. Private use karo.")
    app.run()
